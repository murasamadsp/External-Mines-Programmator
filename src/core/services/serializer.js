...
export class ProgramSerializer {
  ...
  static async decodeBase64(source) {
    ...
    try {
      // LZMA decompress (CyberChef compatible)
      const decompressedData = await LZMACompressor.decompress(data);

      // After LZMA decompression, each byte represents an instruction action code
      const instructions = [];
      for (let i = 0; i < decompressedData.length; i++) {
        const action = decompressedData[i];
        instructions.push(new Instruction(action, "0", null)); // Default label "0"
      }

      return instructions;
    } catch (error) {
      // If LZMA decompression fails, try parsing as uncompressed data
      console.warn("LZMA decompression failed, trying uncompressed parsing");
      try {
        const uncompressedData = data.slice(8); // Skip LZMA header
        return this.parseInstructionsAndLabels(uncompressedData);
      } catch (fallbackError) {
        throw new Error(
          `Failed to decode Base64 v3 program: ${error.message}. Fallback also failed: ${fallbackError.message}`
        );
      }
    }
  }
  ...
  static async encodeBase64(instructions) {
    try {
      ...
      const fullGrid = instructions.slice(); // Assume instructions is already full grid
      while (fullGrid.length < 192) {
        fullGrid.push({ action: 0, label: "0", value: null }); // Add None instructions
      }

      // Create the binary format as per original documentation
      const length = fullGrid.length;

      // Prepare labels string
      const labels = fullGrid
        .map((inst) => {
