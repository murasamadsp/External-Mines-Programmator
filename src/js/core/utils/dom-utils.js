...
export function createSection(title, content, className = "") {
  return createElement(
    "div",
    { className: `controls-section ${className}`.trim() },
    [createElement("h4", {}, title), content],
  );
}
  ...
  if (type === "text") {
    input = createElement(
      "div",
      {
        ...
        "data-type": "text",
        style:
          "outline: none; cursor: text; min-height: 2.5rem; padding: 0.5rem 0.75rem; border: 1px solid #374151; border-radius: 0.375rem; background: white; color: #111827; font-size: 0.875rem; line-height: 1.25rem;",
      },
      value,
    );
...
export function setupGlobalInputProtection() {
  ...
  document.addEventListener(
    ...
    e => {
      ...
      if (isProtected(target)) {
        ...
        if (isFromExtension) {
          console.warn(
            "🚫 Blocked suspected extension focus on protected input",
          );
          e.preventDefault();
          e.stopImmediatePropagation();
          target.blur();
      ...
      if (mutation.type === "attributes") {
        ...
        if (isProtected(element)) {
          ...
          if (
            ...
            console.warn(
              "🔄 Extension tried to change autocomplete, reverting",
            );
  ...
  console.log(
    "🛡️ Smart input protection activated - tracks user interactions to allow legitimate operations",
  );
}
