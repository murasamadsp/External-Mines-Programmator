...
export class ProgramGridComponent {
  ...
  async onDrop(event: DragEvent) {
    ...
    try {
      ...
      if (payload.action !== undefined) {
        ...
        const firstCell = gridElement.querySelector('.program-cell') as HTMLElement;
        if (!firstCell) return;
        const cellWidth = firstCell.offsetWidth;
        const cellHeight = firstCell.offsetHeight;
        const gap = this.getGridGap(gridElement);
  ...
  private getGridGap(gridElement: HTMLElement): number {
    const gridComputedStyle = window.getComputedStyle(gridElement);
    const gap = gridComputedStyle.getPropertyValue('gap');
